...
void main()
{
    ...
    if (colDiffuse.r == 0.0) {
        ...
        if (colDiffuse.g > 0.5) {
            ...
            // float mixing = bloodColor.r;
        
            // mixing = max(mixing, 0.0);
            // mixing = smoothstep(0.0, 0.7, mixing);
            // vec4 bloodedColor = mix(texelColor, bloodColor, mixing);
            if(bloodColor.r  <= (insanity - 0.2) / 0.8) {
                finalColor.rgb = bloodColor.rgb;
            } else {
